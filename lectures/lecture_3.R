...
theme_noaxis<- function(base_size = 12, base_family = "Helvetica") {
...
s<-rep(NA, length(x))
for(i in 1:length(x)){
  m[i]<-mean(sample(population, size=x[i]))
  s[i]<-sd(sample(population, size=x[i]))
}

par(mfrow = c(1,2))
plot(m~x, xlab="size", ylab="mean")
plot(s~x, xlab="size", ylab="sd")